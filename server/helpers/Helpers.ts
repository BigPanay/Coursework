...
//Recieved Online
//https://github.com/vinayak25/nestjs-sample/tree/main/src/core/helpers
...
/**
 *
 * Generates a unique key with nth length.
 *
 * @param nth
 *
 * @returns string
 */
export function randomId(nth?: number): string {
    nth = nth != null ? nth : 0;

    const dateNow: number = Date.now();
    const randomNo = randomNumber(10);

    let randomId = Number(randomNo) + dateNow;

    if (nth > randomId.toString().length) {
        const lenghtLeft = nth - randomId.toString().length;
        let randomIdStr = randomNo.toString().substring(0, lenghtLeft) + randomId.toString();
        randomId = Number(randomIdStr);
    }
...
/**
 * Helper to check if a string is a valid password
 */
export function isPassword(password: string) {
    const passwordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d{3,})(?=.*[#$£^+=!*()@%&{}';:><?]).{8,30}$/g;
...
/**
 * Encode the endCursor
 * The user doesn't need to know what this value means
 * @param valueToEncode 
 */
export function encodeCursor(valueToEncode: string): string {
    return Buffer.from(valueToEncode).toString('base64');
}

/**
 * Decode the endCursor
 * @param valueToDecode 
 */
export function decodeCursor(valueToDecode: string): string {
    return Buffer.from(valueToDecode, 'base64').toString('ascii');
}
